...
## Status update — 2026-09-19, from the session that owns Menu Calculator / Overhead & Manpower / Costing Analysis

Checked Costing Analysis's own `importDataFile()` — named above as the reference implementation — against this standard's own retrofit checklist directly, since "reference implementation" isn't the same as "audited." Found two real gaps even in the reference: no helper text at all next to "Import data" (checklist step 4), and a stale doc-comment above the dispatcher that still said Margin Analysis's export shape was "not yet" handled, when the code three lines below it already handled it (checklist step 5 — "update the doc-comment... to point future readers at this standard" — now done, it points here).

Also found and fixed a real, previously invisible bug this standard's own text doesn't call out explicitly but its stated goal implies should never happen: `costBufferPct` reached `handleSyncPayload` on the live-sync path but was silently dropped on the file-import path for the `menu-calculator` shape — meaning a live-synced dish and the *same* dish imported from a file could quietly disagree on this one field. Fixed; threads through identically on both paths now. Full write-up: `COSTING_ANALYSIS_CHANGE_NOTES.md` (new file, this session's own changelog, parallel to this doc's own `MARGIN_AUDIT_CHANGE_NOTES.md`).
