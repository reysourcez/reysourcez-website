...
**Explicitly deferred (not built) — ingredient-cost-structure comparison.** The idea, captured here so the shape of it isn't lost: when analyzing a photo, offer to identify it as a common recipe (e.g. "chicken rice") and break it down into the standard ingredients for one portion; price each ingredient against average Malaysian costs; sum to a total ingredient cost; then gross that up by the F&B industry's common ~30% ingredient-cost-structure benchmark — `implied_fair_price = ingredient_cost ÷ 0.30`, the same gross-up shape (`target ÷ (1 − rate)`) already used for SST/marketplace fees elsewhere on the site, not a naive markup. That gives a third, supply-side comparison point alongside price-paid and Gemini's existing demand-side "typical market price" estimate — worth it, based on what a fair cost structure would justify, not just what people happen to pay. This would need: a recipe/ingredient breakdown added to the Worker (either an expanded schema or a second Gemini call), and a Malaysia ingredient-price reference table, similar in spirit to the nutrient DV/NRV tables already in `food-worth-calculator.js`. Also raises the `costing-sync.js` question again — a computed ingredient cost is a much closer shape-match to what the other tools already broadcast than the current cost-per-100kcal figures are. Genuinely a bigger build, not a quick add — worth scoping properly when it's time.
