...
- **Multi-column header fields / signature blocks** — a `column` number (1, 2, 3...) on each header field or signature block lets forms with a genuinely side-by-side layout (a payment-voucher header, three signature blocks in a row) render that way instead of everything being forced into one stacked column. `column` is a `required` field in the schema (since 2026-09-23) specifically so the model can't quietly skip the decision and default everything to one stacked column — an early real-world test showed that mattering a lot for exactly this kind of form. Plain fields and checkbox-choice rows render in their real interleaved order within whichever column they belong to, not as two separate passes.
