...
## 2026-09-22 — v1.5.1 (read this first)
...
## Current status (as of 2026-09-22, revised again)
...
| District population & household income (DOSM) | Income ✅ working. Population: **v1.5.0's endpoint fix confirmed BROKEN, 2026-09-22** — both OpenAPI endpoints fail live (see the v1.5.1 section at the top of this doc). v1.5.1 pins the four supported districts' 2023 estimates directly in the Worker instead (`DISTRICT_POPULATION_FALLBACK_2023`), needing a yearly manual refresh rather than a live query |
| Opportunity score, editable weights, diversity index | ✅ Working — 6 weights, see below |
| Competitor strength (Google ratings + on-site read) | ✅ Google ratings: optional, off, paid past 1,000 calls/month. On-site read: counts automatically at 10% (since v1.5.0) — see the settings sheet |
| Gemini plain-English read | ⚠️ Intermittent: Gemini refuses when Cloudflare runs the Worker in a region Google blocks. Workers AI backstop added v1.5.0 — **needs the `AI` binding (deploy step 4)** |
...
**Immediate next step:** deploy v1.5.1 (steps in the section at the top of this doc) and confirm the population card shows a number with the pinned-estimate note under it, for all four towns.
...
| Worker version shown on the page | 1.5.1 | `WORKER_VERSION`, `market-radar-proxy-worker.js` — bump on every deploy |

**Not yet reflected in `market-radar-settings-reference.xlsx`:** the new `DISTRICT_POPULATION_FALLBACK_2023` row above. Nothing already in that spreadsheet is wrong — this is purely an addition, deferred this round to keep the turnaround quick; see Open Items below.
...
- **Add `DISTRICT_POPULATION_FALLBACK_2023` to `market-radar-settings-reference.xlsx`** — straightforward addition, deferred to keep this round's turnaround quick.
